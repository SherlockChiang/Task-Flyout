...
| P1 | 同步测试 | Google/Microsoft task 日期半开区间 policy 和 recurrence 映射 policy 已提取为纯逻辑并测试，覆盖去除时间部分、起止边界、反向区间、Google RRULE、Microsoft pattern type 和创建事件频率映射。继续提取模型映射逻辑，测试全天事件、已完成任务、分页。 |
