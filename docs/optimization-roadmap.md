...
| P2 | 天气权限 | manifest 声明 `location`，天气支持当前位置和自动跟随。 | 在请求位置前增加清晰说明，并展示关闭自动跟随的方法。说明天气数据除 provider 请求外保留在本地。 |
| P2 | 长耗时操作 | 同步、邮件拉取、RSS 刷新、图标包导入、WebView2 缓存清理都可能耗时。 | 使用一致的进度指示，任务运行时禁用重复操作。可行处增加取消。 |
| P2 | 通知设置 | 邮件和提醒通知会路由到应用页面。 | 分开提供提醒、新邮件、天气警报的通知设置，并在 toggle 旁展示当前轮询/提醒间隔。 |
| P2 | 本地化 | README 声明支持英文和简体中文。 | 审计新增和现有硬编码 UI 字符串，尤其是异常 fallback 和状态消息，确保两种语言完整。 |
| P2 | 无障碍 | 自定义 flyout、任务栏天气栏、图标字体和密集列表可能存在无障碍缺口。 | 为纯图标按钮增加 accessible name/tooltip，验证键盘导航、高对比度、缩放和屏幕阅读器标签。 |
