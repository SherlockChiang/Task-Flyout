...
| P2-09 | IN PROGRESS | Performance | Bound RSS host-gate retention and reduce payload copies. | Host coordination storage is fixed at 64 gates with global concurrency 4; image/feed transfer still needs fewer full-buffer copies. | `perf: bound RSS host concurrency gates` |
